...
public class JBE implements Command {
	...
	public JBE(String label) {
		...
		if (CompilerUtils.isJumpBack(label)) {
			...
			String dif = AssemblerHelper.subHexFromHex(offset, CompilerUtils.getLabelOffset(label));
			code += StringUtils.beautifyCode(AssemblerHelper.subHexFromHex("FE", dif) + " ", 2); // FF - (jbeOff + labelOff) - 1
			this.offset = 2;
		} else {
			code = StringConstants.SECOND_GT;
			this.offset = 4;
			this.jbeOffset = Compiler.getOffset();
		}
	}
	...
	public void updateCode() { // in case of forward jump
		code = "76 ";
		String offset = CompilerUtils.getOffset(this.jbeOffset);
